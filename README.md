...
- `admin`：用户管理、来源启停、全量抓取批次、历史回填与来源能力审计。
...
当前测试覆盖前端生产构建/SSR、静态数据回退禁用、登录/刷新/锁定、角色权限、审计、事件级去重、年度筛选、政策与 KA 识别、批量抓取、回填断点、来源状态、手动导入、RSS/Sitemap 标准化以及空库和既有库 Alembic 升级。
